(function () {
    ...
    ko.bindingHandlers.textbox = {
        init: function (element, valueAccessor, allBindingsAccessor, viewModel) {
            ...
            var onChange, options, value;
            value = ko.utils.unwrapObservable(valueAccessor());//得到元素上绑定的值
            //console.log(value);
            ...
            return utils.component.bindDisposeEvent(element, "textbox");

        },
        update: function (element, valueAccessor, allBindingsAccessor, viewModel) {
            // This will be called once when the binding is first applied to an element,
            //这将调用一次，当结合首先被应用到一个元素
            ...
            $(element).textbox('setValue', value);
            if ($(element).textbox('getValue')) {
                return $(element).removeClass("validatebox-invalid");
            } else {
                if (($(element).textbox('options').required)) {
                    return $(element).addClass("validatebox-invalid");
                }
            }
        }
    ...
    ko.bindingHandlers.textboxValue = {
        init: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
            ...
            value = valueAccessor();
            if (value() == null) {
                curValue = $(element).textbox('getValue');
                if (curValue) {
                    value(curValue);
                }
            }
