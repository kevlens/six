...
*/
(function (f) {
    var A = f.Chart, t = f.addEvent, B = f.removeEvent, l = f.createElement, o = f.discardElement, v = f.css, k = f.merge, r = f.each, p = f.extend, D = Math.max, j = document, C = window, E = f.isTouchDevice, F = f.Renderer.prototype.symbols, s = f.getOptions(), y; p(s.lang, { printChart: "打印图表", downloadPNG: "下载PNG图片", downloadJPEG: "下载JPEG图片", downloadPDF: "下载PDF文件", downloadSVG: "下载SVG矢量图片", contextButtonTitle: "图表菜单" }); s.navigation = { menuStyle: { border: "1px solid #A0A0A0",
    ...
    }; s.exporting = { type: "image/png", url: "http://localhost:8080/ExportingServer-java-strut2/export", buttons: { contextButton: { menuClassName: "highcharts-contextmenu",
...
/px$/.test(c) && parseInt(c, 10) || 900; h = g.exporting.sourceHeight || g.chart.height || /px$/.test(h) && parseInt(h, 10) || 600; p(g.chart, { animation: !1, renderTo: b, forExport: !0, width: c, height: h }); g.exporting.enabled = !1; g.series = []; r(a.series, function (a) { z = k(a.options, { animation: !1, enableMouseTracking: !1, showCheckbox: !1, visible: a.visible }); z.isInternal || g.series.push(z) }); d = new f.Chart(g, a.callback); r(["xAxis", "yAxis"], function (b) {
    r(a[b], function (a, c) {
